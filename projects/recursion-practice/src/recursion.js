...
var capitalizeFirst = function(input, output = []) {
  ...
  output.push(input[0].charAt(0).toUpperCase().concat(input[0].slice(1)))
...
var compress = function(list, list2 = []) {
  if(list.length === 0){
    return list2
  }
  if(list[0] !== list2[list2.length - 1]){
    list2.push(list[0])
  }
  return compress(list.slice(1), list2)
};
...
var minimizeZeroes = function(array, arr2 = []) {
  if(array.length === 0){
    return arr2
  }
  if(array[0] !== 0){
  ...
  if(array[0] === 0 && array[0] !== arr2[arr2.length - 1]){
    arr2.push(array[0])
  }
  return minimizeZeroes(array.slice(1), arr2)
};
...
// alternateSign([-2,-7,8,3,-1,4]) // [2,-7,8,-3,1,-4]
var alternateSign = function(array, arr2 = []) {
  if(array.length === 0){
    return arr2
  }
  if(arr2.length === 0){//the first number will be positive 
    if(array[0] < 0){
      arr2.push(array[0] * -1)
      return alternateSign(array.slice(1), arr2)
    }
    else {
      arr2.push(array[0])
      return alternateSign(array.slice(1), arr2)
  ...
  if(arr2.length % 2 === 1){//if the previous number was odd the next number will be negative
    if(array[0] < 0){
      arr2.push(array[0])
      return alternateSign(array.slice(1), arr2)
    }
  ...
  if(arr2.length % 2 === 0){//if the previous number was even the next number will be positive
    if(array[0] < 0){
      arr2.push(array[0] * -1)
      return alternateSign(array.slice(1), arr2)
    }
